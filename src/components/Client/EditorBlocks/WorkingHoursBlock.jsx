        ...
        return (
            <div key={idx} style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '16px', padding: '12px', background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '8px', position: 'relative' }}>
              ...
              <button 
                ...
                aria-label={`Remove working hours for ${wh.days || `entry ${idx + 1}`}`}
                title={`Remove working hours for ${wh.days || `entry ${idx + 1}`}`}
              >✕</button>
                ...
                <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '4px' }}>
                   ...
                   {dayPresets.includes(wh.days) || !wh.days ? (
                     ...
                     <div style={{ display: 'flex', gap: '4px' }}>
                       ...
                       <Button variant="secondary" size="icon" aria-label="Reset to default preset" title="Reset to default preset" onClick={(e) => { e.preventDefault(); updateField("days", "Monday - Friday"); }}>↺</Button>
                     </div>
                   )}
                </div>
