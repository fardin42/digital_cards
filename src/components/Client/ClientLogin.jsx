        ...
        <div className="card-form" style={{maxWidth: '400px', margin: '0 auto'}}>
          ...
          <form onSubmit={verifyAccount} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <div className="input-group">
              ...
              <input id="otpCode" autoComplete="one-time-code" disabled={loading} required type="text" placeholder="123456" value={otp} onChange={e => setOtp(e.target.value)} style={{ padding: '12px', fontSize: '1.2rem', letterSpacing: '4px', textAlign: 'center', boxSizing: 'border-box', width: '100%' }} />
            </div>
        ...
        <form onSubmit={submitCredentials} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          {isSignUp && (
             <div className="input-group">
               ...
               <input id="fullName" autoComplete="name" disabled={loading} required type="text" placeholder="John Doe" value={form.name} onChange={e => setForm({...form, name: e.target.value})} style={{ padding: '12px', boxSizing: 'border-box', width: '100%' }} />
             </div>
          )}
          <div className="input-group">
            <label htmlFor="emailAddress">Email Address</label>
            <input id="emailAddress" autoComplete="email" disabled={loading} required type="email" placeholder="registered@email.com" value={form.email} onChange={e => setForm({...form, email: e.target.value})} style={{ padding: '12px', boxSizing: 'border-box', width: '100%' }} />
          </div>
          <div className="input-group">
             <label htmlFor="password">Password</label>
             <input id="password" autoComplete={isSignUp ? "new-password" : "current-password"} disabled={loading} required type="password" placeholder="••••••••" value={form.password} onChange={e => setForm({...form, password: e.target.value})} style={{ padding: '12px', boxSizing: 'border-box', width: '100%' }} />
          </div>
