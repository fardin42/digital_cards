                    ...
                    <td className="actions-cell">
                      {isEditing ? (
                        <div className="edit-actions">
                          <button className="btn-save-edit" onClick={() => handleSaveEdit(client)} title={`Save changes for client ${client.name || 'Unknown'}`} aria-label={`Save changes for client ${client.name || 'Unknown'}`}><Save size={16} /></button>
                          <button className="btn-cancel-edit" onClick={() => setEditingIdx(null)} title={`Cancel editing for client ${client.name || 'Unknown'}`} aria-label={`Cancel editing for client ${client.name || 'Unknown'}`}><X size={16} /></button>
                        </div>
                      ) : (
                        <div className="row-action-group">
                          ...
                          {card && (
                            <button
                              ...
                              title={`${card.status === 'active' ? 'Suspend' : 'Activate'} client ${client.name || 'Unknown'}`}
                          ...
                          <div className="dropdown-container" ref={menuOpenIdx === i ? menuRef : null}>
                            <button className="action-btn" title={`More actions for client ${client.name || 'Unknown'}`} aria-label={`More actions for client ${client.name || 'Unknown'}`} onClick={(e) => { e.stopPropagation(); setMenuOpenIdx(menuOpenIdx === i ? null : i); }}>
