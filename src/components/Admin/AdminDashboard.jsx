    ...
    <div className="confirm-overlay" onClick={onCancel}>
      <div className="confirm-box" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="confirm-actions">
          ...
          <button
            className={`confirm-ok-btn ${danger ? "confirm-danger" : ""}`}
            onClick={onConfirm}
          >
...
export default function AdminDashboard({
  ...
  const activeClients =
    clients?.filter((c) => c.cards?.[0]?.status === "active").length || 0;
  const suspendedClients =
    clients?.filter((c) => c.cards?.[0]?.status === "suspended").length || 0;
  const pendingClients =
    clients?.filter(
      (c) => !c.cards?.[0]?.status || c.cards?.[0]?.status === "pending",
    ).length || 0;
  ...
  const newOnboards =
    clients?.filter((c) => new Date(c.created_at) >= startOfMonth).length || 0;
    ...
    <div className="admin-layout">
        ...
        <div className="sidebar-footer">
          <div className="sidebar-stats-mini">
            <div className="mini-stat">
              <span>{totalClients}</span> Total
            </div>
            <div className="mini-stat">
              <span>{activeClients}</span> Active
            </div>
          </div>
        </div>
      </aside>
      ...
      <main className="admin-main">
        ...
        <div className="table-filters">
          ...
          <span className="filter-pill filter-active-pill">
            Active ({activeClients})
          </span>
          <span className="filter-pill filter-suspended-pill">
            Suspended ({suspendedClients})
          </span>
          <span className="filter-pill filter-pending-pill">
                ...
                return (
                  ...
                  >
                    ...
                    <td className="date-cell">
                      {sub
                        ? new Date(sub.next_bill_date).toLocaleDateString(
                            "en-US",
                      ...
                      {isEditing ? (
                        ...
                        <div className="row-action-group">
                          ...
                          <div
                            ...
                            ref={menuOpenIdx === i ? menuRef : null}
                          >
                            <button
                            ...
                            {menuOpenIdx === i && (
                              <div className="dropdown-menu">
                                <button
                                  onClick={(e) => handleStartEdit(client, i, e)}
        ...
        <div className="stat-block">
          ...
          <div className="stat-legend">
            <div className="legend-item">
              <span className="dot dot-green"></span> ₹{monthlyIncome || 0}{" "}
              <span className="light-text">Collected</span>
            </div>
          </div>
        </div>
        ...
        <div className="stat-block">
          ...
          <div className="progress-bar-container">
            <div
              ...
              style={{
                width: `${totalClients > 0 ? (activeClients / totalClients) * 100 : 0}%`,
              }}
            ></div>
            <div
              className="progress-bar bar-red"
              style={{
                width: `${totalClients > 0 ? (suspendedClients / totalClients) * 100 : 0}%`,
              }}
            ></div>
            <div
              className="progress-bar bar-grey"
              style={{
                width: `${totalClients > 0 ? (pendingClients / totalClients) * 100 : 0}%`,
              }}
            ></div>
          </div>
          <ul className="status-list">
            <li>
              <span className="dot dot-green"></span> Active{" "}
              <span className="status-value">{activeClients}</span>
            </li>
            <li>
              <span className="dot dot-red"></span> Suspended{" "}
              <span className="status-value">{suspendedClients}</span>
            </li>
            <li>
              <span className="dot dot-grey"></span> Pending{" "}
              <span className="status-value">{pendingClients}</span>
            </li>
          </ul>
        </div>
