                    ...
                    <td className="actions-cell">
                      {isEditing ? (
                        <div className="edit-actions">
                          <button className="btn-save-edit" aria-label={`Save edits for ${client.name || 'Unknown'}`} onClick={() => handleSaveEdit(client)} title="Save"><Save size={16} /></button>
                          <button className="btn-cancel-edit" aria-label={`Cancel editing ${client.name || 'Unknown'}`} onClick={() => setEditingIdx(null)} title="Cancel"><X size={16} /></button>
                        </div>
                      ) : (
                        <div className="row-action-group">
                          ...
                          {card && (
                            <button
                              ...
                              aria-label={card.status === 'active' ? `Suspend client ${client.name || 'Unknown'}` : `Activate client ${client.name || 'Unknown'}`}
                            >
                          ...
                          <div className="dropdown-container" ref={menuOpenIdx === i ? menuRef : null}>
                            <button
                              ...
                              aria-label={`Actions for ${client.name || 'Unknown'}`}
                              title={`Actions for ${client.name || 'Unknown'}`}
                              aria-haspopup="true"
